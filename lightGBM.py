...
class lightGBM(object):
    ...
    def fit_model(self):
        self.LGBM= lgb.LGBMRegressor(learning_rate=0.09,max_depth=-5,random_state=42)
        self.LGBM.fit(self.X_train, self.Y_train)
        save_path = '/home/sevin/Desktop/projects/TravelTime_Prediction/HBRegression/TT_pred_wHBR/trained_models/{}/lightGBM/'.format(self.data_type)
        if self.direct:
            with open(save_path + 'model1_MN_LGBM_direct.pickle', 'wb') as buff:
                pickle.dump(self.LGBM, buff)
        else:
            with open(save_path + 'model1_MN_LGBM.pickle', 'wb') as buff:
                pickle.dump(self.LGBM, buff)
    
    def load_model(self):
        save_path = '/home/sevin/Desktop/projects/TravelTime_Prediction/HBRegression/TT_pred_wHBR/trained_models/{}/lightGBM/'.format(self.data_type)
        if self.direct:
            with open(save_path + 'model1_MN_LGBM_direct.pickle', 'rb') as buff:
                self.LGBM = pickle.load(buff)
        else:
            with open(save_path + 'model1_MN_LGBM.pickle', 'rb') as buff:
                self.LGBM = pickle.load(buff)
    ...
    def test_model(self):
        self.Y_preds_tst = self.LGBM.predict(self.X_test)
        ...
        self.Y_preds_tr = self.LGBM.predict(self.X_train)
    ...
    def save_predictions(self):
        ...
        if self.direct:
            self.train_df['LGBM_tt_predMean_direct'] = self.Y_preds_tr/60.
            # self.train_df['LRegression_tt_predStd_direct'] = self.std_preds_tr/60.

            self.test_df['LGBM_tt_predMean_direct'] = self.Y_preds_tst/60.
