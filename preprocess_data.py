...
class data_processing(object):
    ...
    def concat_data(self):
        ...
        if self.data_type == 'taxi':
            ...
            num_data = 15
            for i in range(num_data):
                    data_path = '/home/sevin/Desktop/projects/TravelTime_Prediction/HBRegression/TT_pred_wHBR/data/raw_data/taxi/taxi_/' + 'taxi_data_m123_Tue_Thr_wSPdist_{}.txt'.format(i)
            ...
            new_gdf.to_csv('/home/sevin/Desktop/projects/TravelTime_Prediction/HBRegression/TT_pred_wHBR/data/raw_data/taxi/taxi_/' + 'taxi_data_m123_Tue_Thr_wSPdist_processed.txt')
    ...
    def get_atoms_of_regions(self): # create dictionary that maps each atom to the region it belongs 
        ...
        return self.atoms_polygon_mn, self.atomsCenters_mn , dict(zip(self.atoms_polygon_mn['ATOM'],self.atoms_polygon_mn['DISP_FREQ']))


    def clean(self):
        # print(self.data[['travel_time', 'unitDist_TT_hpm', 'highway']].describe())
        # exit()
        ...
        if self.data_type == 'amb':
            tresh = 3
        elif self.data_type == 'taxi':
            tresh = 1.5
        else:
            raise NotImplementedError
        self.data = self.data[self.data['unitDist_TT_hpm']<= 1/tresh] # will drop 4024(4013) data points/trips if we assume at min avg speed is 1.5 mph
        print('*** droped {} rows due to speeds smaller than {}mph'.format(data_size - len(self.data), tresh))
    ...
    def merge_weather_data(self):
        ...
        # # #         return [0]
        # # #     else:
        # # #         return list(weat_subset['HourlyPresentWeatherType'].unique())
    
        self.data['adverse_weather'] = self.data.apply(lambda x: fun_(x), axis=1)
        # # # self.data['weather_type'] = self.data.apply(lambda x: fun2(x), axis=1)
        # # # tmp  =list(self.data['weather_type'])
        # # # import itertools
        # # # import collections
        # # # print(collections.Counter(np.array(list(itertools.chain.from_iterable(tmp)))))
        # # # exit()
    ...
    def calculate_stats(self):
        ...
        # plt.hist(self.data['path_avg_bwness'], bins=25)
        # plt.show()
        # exit()
