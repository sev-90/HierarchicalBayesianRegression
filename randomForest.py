...
class RForest(object):
    ...
    def fit_model(self):
        self.RF= RandomForestRegressor(n_estimators = 20, random_state = 0)
        self.RF.fit(self.X_train, self.Y_train)
        save_path = '/home/sevin/Desktop/projects/TravelTime_Prediction/HBRegression/TT_pred_wHBR/trained_models/{}/RandomForest/'.format(self.data_type)
        if self.direct:
            with open(save_path + 'model1_MN_RForest_direct.pickle', 'wb') as buff:
                pickle.dump(self.RF, buff)
        else:
            with open(save_path + 'model1_MN_RForest.pickle', 'wb') as buff:
                pickle.dump(self.RF, buff)
    
    def load_model(self):
        save_path = '/home/sevin/Desktop/projects/TravelTime_Prediction/HBRegression/TT_pred_wHBR/trained_models/{}/RandomForest/'.format(self.data_type)
        if self.direct:
            with open(save_path + 'model1_MN_RForest_direct.pickle', 'rb') as buff:
                self.RF = pickle.load(buff)
        else:
            with open(save_path + 'model1_MN_RForest.pickle', 'rb') as buff:
                self.RF = pickle.load(buff)
    ...
    def test_model(self):
        self.Y_preds_tst = self.RF.predict(self.X_test)
        ...
        self.Y_preds_tr = self.RF.predict(self.X_train)
    ...
    def save_predictions(self):
        ...
        if self.direct:
            self.train_df['RForest_tt_predMean_direct'] = self.Y_preds_tr/60.
            # self.train_df['LRegression_tt_predStd_direct'] = self.std_preds_tr/60.

            self.test_df['RForest_tt_predMean_direct'] = self.Y_preds_tst/60.
